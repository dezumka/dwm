...
enum { SchemeNorm, SchemeSel, SchemeTitle, SchemeTag, SchemeTag1, SchemeTag2,
       SchemeTag3, SchemeTag4, SchemeTag5, SchemeLayout }; /* color schemes */
  ...
  if(floatbar){
    ...
    drw_setscheme( drw, scheme[occ & 1 << i ? (m->colorfultag ? tagschemes[i] : SchemeSel): SchemeTag]);
		...
		if (ulineall || m->tagset[m->seltags] & 1 << i) /* if there are conflicts, just move these lines directly underneath both 'drw_setscheme' and 'drw_text' :) */
			drw_rect(drw, x + ulinepad, 0, w - (ulinepad * 2), ulinestroke, 1, 0);
